...
public class MetricsResourceTool {
    ...
    @LlmTool(name = "inspectPodResourceHealth", description = """
    ...
    @LlmTool(name = "inspectWorkloadResourceHealth", description = """
    ...
    public String inspectWorkloadResourceHealth(String namespace, String kind, String name) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("=== WORKLOAD RESOURCE HEALTH: ")
                .append(kind)
                .append(" ")
                .append(namespace)
                .append("/")
        ...
        sb.append("OOMKilled pods: ")
                .append(oomCount)
                .append("/")
                .append(pods.size())
                .append("\n");
        sb.append("Total restarts across fleet: ").append(restartTotal).append("\n");
        sb.append("Pods missing memory/cpu limits: ")
                .append(missingLimits)
                .append("/")
                .append(pods.size())
                .append("\n");
        sb.append("Pods missing memory/cpu requests: ")
                .append(missingRequests)
                .append("/")
                .append(pods.size())
                .append("\n");
        sb.append("BestEffort QoS pods: ")
                .append(bestEffortCount)
                .append("/")
                .append(pods.size())
                .append("\n");
        return sb.toString();
    ...
    @LlmTool(name = "inspectNamespaceResourceConstraints", description = """
    ...
    public String inspectNamespaceResourceConstraints(String namespace) {
        ...
        List<ResourceQuota> quotas =
                kubernetesClient.resourceQuotas().inNamespace(namespace).list().getItems();
        if (quotas.isEmpty()) {
            ...
            for (ResourceQuota q : quotas) {
                ...
                Map<String, Quantity> hard = Optional.ofNullable(q.getStatus())
                        .map(ResourceQuotaStatus::getHard)
                        .orElse(Collections.emptyMap());
                Map<String, Quantity> used = Optional.ofNullable(q.getStatus())
                        .map(ResourceQuotaStatus::getUsed)
                        .orElse(Collections.emptyMap());
        ...
        List<LimitRange> limitRanges =
                kubernetesClient.limitRanges().inNamespace(namespace).list().getItems();
        ...
        quotas.forEach(q -> {
            ...
            hard.forEach((key, hardQ) -> {
                if (isNearLimit(used.get(key), hardQ)) {
                    sb.append("  - Quota '")
                            .append(key)
                            .append("' is near or at its hard limit — ")
    ...
    @LlmTool(name = "inspectNodeResourcePressure", description = """
    ...
    public String inspectNodeResourcePressure(String nodeName) {
        ...
        for (Node node : nodes) {
            ...
            Map<String, Quantity> allocatable =
                    Optional.ofNullable(status).map(NodeStatus::getAllocatable).orElse(Collections.emptyMap());
            Map<String, Quantity> capacity =
                    Optional.ofNullable(status).map(NodeStatus::getCapacity).orElse(Collections.emptyMap());

            sb.append("  capacity:    cpu=")
                    .append(quantityStr(capacity.get("cpu")))
                    .append(" memory=")
                    .append(quantityStr(capacity.get("memory")))
                    .append(" ephemeral-storage=")
                    .append(quantityStr(capacity.get("ephemeral-storage")))
                    .append("\n");
            sb.append("  allocatable: cpu=")
                    .append(quantityStr(allocatable.get("cpu")))
                    .append(" memory=")
                    .append(quantityStr(allocatable.get("memory")))
                    .append(" ephemeral-storage=")
                    .append(quantityStr(allocatable.get("ephemeral-storage")))
                    .append("\n");

            List<NodeCondition> conditions =
                    Optional.ofNullable(status).map(NodeStatus::getConditions).orElse(Collections.emptyList());
            sb.append("  conditions:\n");
            for (NodeCondition c : conditions) {
                boolean isTrue = "True".equals(c.getStatus());
                boolean isPressureOrNotReady =
                        List.of("MemoryPressure", "DiskPressure", "PIDPressure").contains(c.getType())
                                ? isTrue
            ...
            if (!taints.isEmpty()) {
                sb.append("  taints: ")
                        .append(taints.stream()
                                .map(t -> t.getKey() + "=" + t.getValue() + ":" + t.getEffect())
                                .collect(Collectors.joining(", ")))
                        .append("\n");
            }

            long podCount = kubernetesClient
                    .pods()
                    .inAnyNamespace()
                    .withField("spec.nodeName", nName)
                    .list()
                    .getItems()
                    .size();
    ...
    @LlmTool(name = "clusterResourceSummary", description = """
    ...
    public String clusterResourceSummary() {
        ...
        sb.append(String.format(
                "%-40s  %12s  %12s  %12s  %12s  %12s  %12s  %6s  %6s%n",
                "NODE",
                "ALLOC-CPU(m)",
                "USED-CPU(m)",
                "FREE-CPU(m)",
        ...
        for (Node node : nodes) {
            ...
            String cpuPct =
                    usedCpuM >= 0 && allocCpuM > 0 ? String.format("%5.1f%%", 100.0 * usedCpuM / allocCpuM) : "  n/a";
            String memPct =
                    usedMemB >= 0 && allocMemB > 0 ? String.format("%5.1f%%", 100.0 * usedMemB / allocMemB) : "  n/a";

            sb.append(String.format(
                    "%-40s  %12d  %12s  %12s  %12d  %12s  %12s  %6s  %6s%n",
                    nName,
                    allocCpuM,
        ...
        String totalCpuPct = hasLiveMetrics && totalAllocCpuM > 0
                ? String.format("%5.1f%%", 100.0 * totalUsedCpuM / totalAllocCpuM)
                : "  n/a";
        String totalMemPct = hasLiveMetrics && totalAllocMemB > 0
                ? String.format("%5.1f%%", 100.0 * totalUsedMemB / totalAllocMemB)
                : "  n/a";
    ...
    @LlmTool(name = "currentResourceUsage", description = """
    ...
    public String currentResourceUsage(String namespaceOrEmpty) {
        ...
        try {
            ...
            if (hasNamespace) {
                podMetricsList =
                        kubernetesClient.top().pods().metrics(namespaceOrEmpty).getItems();
            } else {
                podMetricsList = kubernetesClient.top().pods().metrics().getItems();
            }

            if (podMetricsList.isEmpty()) {
                sb.append("[Pod Metrics] no data returned — metrics-server may not have scraped yet\n");
            } else {
                sb.append("[Pod Metrics] namespace=")
                        .append(hasNamespace ? namespaceOrEmpty : "ALL")
                        .append("\n");
            ...
            sb.append("[Pod Metrics] metrics_unavailable — ")
                    .append(metricsUnavailableHint(e))
                    .append("\n");
    ...
    private void appendPodEvidence(StringBuilder sb, Pod pod) {
        ...
        sb.append("  phase=").append(phase).append("  qosClass=").append(qos).append("\n");
        ...
        Map<String, ContainerStatus> statusByName =
        ...
        for (Container c : containers) {
            ...
            if (res == null) {
                ...
                Map<String, Quantity> req =
                        Optional.ofNullable(res.getRequests()).orElse(Collections.emptyMap());
                Map<String, Quantity> lim = Optional.ofNullable(res.getLimits()).orElse(Collections.emptyMap());
                sb.append("    requests: cpu=")
                        .append(quantityStr(req.get("cpu")))
                        .append(" memory=")
                        .append(quantityStr(req.get("memory")))
                        .append("\n");
                sb.append("    limits:   cpu=")
                        .append(quantityStr(lim.get("cpu")))
                        .append(" memory=")
                        .append(quantityStr(lim.get("memory")))
                        .append("\n");
            }
            ...
            if (cs != null) {
                ...
                if (state != null && state.getWaiting() != null) {
                    sb.append("    state: Waiting reason=")
                            .append(state.getWaiting().getReason())
                            .append("\n");
                }
                if (state != null && state.getTerminated() != null) {
                    ContainerStateTerminated t = state.getTerminated();
                    sb.append("    state: Terminated reason=")
                            .append(t.getReason())
                            .append(" exitCode=")
                            .append(t.getExitCode())
                            .append("\n");
                }
                ContainerState last = cs.getLastState();
                if (last != null && last.getTerminated() != null) {
                    ContainerStateTerminated lt = last.getTerminated();
                    sb.append("    lastTermination: reason=")
                            .append(lt.getReason())
                            .append(" exitCode=")
                            .append(lt.getExitCode())
                            .append(" finishedAt=")
                            .append(lt.getFinishedAt())
                            .append("\n");
                    if ("OOMKilled".equals(lt.getReason())
                            || Integer.valueOf(137).equals(lt.getExitCode())) {
        ...
        for (Container c : containers) {
                ...
                if (last != null && last.getTerminated() != null) {
                    ...
                    if ("OOMKilled".equals(lt.getReason())
                        ...
                        sb.append("  - Container '")
                                .append(c.getName())
                                .append(
                                        "' was OOMKilled. Memory limit is too tight or the application has a memory leak.\n");
                        anySuspicion = true;
                    }
                }
            ...
            if (res == null || res.getLimits() == null || res.getLimits().isEmpty()) {
                sb.append("  - Container '")
                        .append(c.getName())
                        .append(
                                "' has no resource limits — it can consume all node memory and trigger OOM on the node.\n");
                anySuspicion = true;
            }
            if (res == null || res.getRequests() == null || res.getRequests().isEmpty()) {
                sb.append("  - Container '")
                        .append(c.getName())
                        .append(
                                "' has no resource requests — QoS is BestEffort, first to be evicted under pressure.\n");
                anySuspicion = true;
            }
        }
    ...
    private boolean hasPodOOMKilled(Pod pod) {
        return Optional.ofNullable(pod.getStatus())
                ...
                .anyMatch(cs -> {
                    ...
                    return "OOMKilled".equals(t.getReason())
                            || Integer.valueOf(137).equals(t.getExitCode());
                });
    }
    ...
    private boolean hasMissingLimits(Pod pod) {
        return Optional.ofNullable(pod.getSpec()).map(PodSpec::getContainers).orElse(Collections.emptyList()).stream()
                .anyMatch(c ->
                        c.getResources() == null || !notEmpty(c.getResources().getLimits()));
    }

    private boolean hasMissingRequests(Pod pod) {
        return Optional.ofNullable(pod.getSpec()).map(PodSpec::getContainers).orElse(Collections.emptyList()).stream()
                .anyMatch(c ->
                        c.getResources() == null || !notEmpty(c.getResources().getRequests()));
    }
    ...
    private long parseToBytes(Quantity q) {
        ...
        try {
            ...
            // plain bytes or scientific notation from metrics-server (e.g. "123456789n" nanocores won't appear for
            // memory)
